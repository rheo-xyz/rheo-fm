...
contract RheoFactoryTest is BaseTest {
    ...
    function test_RheoFactory_admin() public view {
        assertTrue(sizeFactory.hasRole(0x00, owner));
    }

    function test_RheoFactory_set_candidate() public {
        IRheo candidate = IRheo(makeAddr("candidate"));
        vm.expectRevert();
    ...
    function test_RheoFactory_set_2_existing_markets_1() public {
        assertEq(address(sizeFactory.getMarket(0)), address(size));
        assertEq(sizeFactory.getMarketDescriptions()[0], string.concat("Rheo | WETH | USDC | 130 | ", VERSION));

        shouldDeployRheoFactory = false;
        ...
        assertEq(sizeFactory.getMarketDescriptions()[1], string.concat("Rheo | CTK | BTK | 130 | ", VERSION));
    }

    function test_RheoFactory_set_2_existing_markets_add_3rd_market() public {
        assertEq(address(sizeFactory.getMarket(0)), address(size));
        assertEq(sizeFactory.getMarketDescriptions()[0], string.concat("Rheo | WETH | USDC | 130 | ", VERSION));
        ...
        assertEq(sizeFactory.getMarketDescriptions()[2], string.concat("Rheo | stETH | WETH | 125 | ", VERSION));
    }

    function test_RheoFactory_createPriceFeed() public {
    ...
    function test_RheoFactory_createMarket_unauthorized() public {
        vm.prank(alice);
        vm.expectRevert(
    ...
    function test_RheoFactory_createPriceFeed_unauthorized() public {
        MockV3Aggregator aggregator1 = new MockV3Aggregator(2, 1000e2);
        MockV3Aggregator aggregator2 = new MockV3Aggregator(2, 1e2);
        MockERC20 baseToken = new MockERC20("Base Token", "BT", 18);
        MockERC20 quoteToken = new MockERC20("Quote Token", "QT", 18);
    ...
    function test_RheoFactory_createBorrowTokenVault_unauthorized() public {
        vm.prank(alice);
        vm.expectRevert(
            abi.encodeWithSelector(IAccessControl.AccessControlUnauthorizedAccount.selector, address(alice), 0x00)
        );
    ...
    function test_RheoFactory_getMarketsCount() public {
    ...
    function test_RheoFactory_createMarket_revert_on_unauthorized() public {
    ...
    function test_RheoFactory_initialize_multiple_markets_and_getDescriptions() public {
        ...
        assertEq(descriptions[1], string.concat("Rheo | MTA | MTB | 120 | ", VERSION));
    }

    function test_RheoFactory_version() public view {
        string memory version = sizeFactory.version();
        assertEq(version, VERSION);
    }

    function test_RheoFactory_setRheoImplementation() public {
        vm.prank(owner);
        ...
        vm.prank(owner);
        sizeFactory.setRheoImplementation(newImplementation);
        assertEq(sizeFactory.sizeImplementation(), newImplementation);

        vm.prank(owner);
        sizeFactory.createMarket(f, r, o, d);
        assertEq(RheoMock(address(sizeFactory.getMarket(1))).v(), 2);
    }

    function test_RheoFactory_setNonTransferrableRebasingTokenVaultImplementation() public {
        vm.prank(owner);
        vm.expectRevert(abi.encodeWithSelector(Errors.NULL_ADDRESS.selector));
    ...
    function test_RheoFactory_upgade() public {
