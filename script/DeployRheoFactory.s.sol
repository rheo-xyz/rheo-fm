...
contract DeployRheoFactoryScript is BaseScript, Networks, Deploy {
    ...
    function run() public parseEnv broadcast {
        ...
        if (shouldUpgrade) {
            ...
            deployments.push(Deployment({name: "RheoFactory-implementation", addr: address(implementation)}));
            deployments.push(Deployment({name: "RheoFactory-proxy", addr: address(proxy)}));
            parameters.push(Parameter({key: "owner", value: Strings.toHexString(owner)}));

            console.log("[RheoFactory v1.5] deployed\n");

            for (uint256 i = 0; i < deployments.length; i++) {
                console.log("[RheoFactory v1.5] Deployment: ", deployments[i].name, "\t", address(deployments[i].addr));
            }
            for (uint256 i = 0; i < parameters.length; i++) {
                console.log("[RheoFactory v1.5] Parameter:  ", parameters[i].key, "\t", parameters[i].value);
            }
